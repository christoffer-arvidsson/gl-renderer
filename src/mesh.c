...
Mesh mesh_alloc(Region *allocator, size_t vertex_capacity) {
    Mesh mesh = {.vertices = vertex_buffer_alloc(allocator, vertex_capacity)};
    return mesh;
}
...
void mesh_renderer_draw(const MeshRenderer *renderer,
    ...
    const VertexBuffer *vertices = &renderer->mesh.vertices;
    glBufferData(GL_ARRAY_BUFFER, vertices->count * sizeof(vertices->data[0]),
    ...
    shader_set_vec3f(&renderer->shader, "material.ambient",
                     renderer->material.ambient);
    shader_set_vec3f(&renderer->shader, "material.specular",
                     renderer->material.specular);
    shader_set_vec3f(&renderer->shader, "material.diffuse",
                     renderer->material.diffuse);
    shader_set_float(&renderer->shader, "material.shininess",
                     renderer->material.shininess);

    shader_set_vec3f(&renderer->shader, "light.ambient",
                     renderer->light.ambient);
    shader_set_vec3f(&renderer->shader, "light.specular",
                     renderer->light.specular);
    shader_set_vec3f(&renderer->shader, "light.diffuse",
                     renderer->light.diffuse);
    shader_set_vec3f(&renderer->shader, "light.position",
